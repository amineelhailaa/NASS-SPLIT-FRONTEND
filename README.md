...
- **2026-04-22** — **`scrollToBottom` and `requestAnimationFrame`:** wrapping `el.scrollTop = el.scrollHeight` in `requestAnimationFrame` ensures the scroll runs after the browser has completed layout and paint — necessary when a parent container switches CSS overflow on tab change, which can make `scrollHeight` incorrect if read during Vue's `nextTick` alone.
