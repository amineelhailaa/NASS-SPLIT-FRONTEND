...
function handleClickOutside(e) {
  if (payerRef.value && !payerRef.value.contains(e.target)) {
...
onMounted(async () => {
  ...
  try {
    ...
    participants.value = members.value
...
const onSubmit = handleSubmit(async () => {
  ...
  if (splitStrategy.value === 'fixed') {
    const totalCents = Math.round(fixedTotal.value * 100)
  ...
  if (splitStrategy.value === 'percentage') {
    if (Math.abs(percentageTotal.value - 100) > 0.1) {
  ...
  <div class="flex flex-col gap-6" style="font-family: 'Plus Jakarta Sans', sans-serif">
    ...
    <form v-else @submit.prevent="onSubmit" class="flex flex-col gap-6">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <div class="relative">
            ...
            <span
              class="material-symbols-outlined absolute right-5 top-1/2 -translate-y-1/2 text-cerulean-400 text-lg pointer-events-none"
              >expand_more</span
            >
          </div>
        </div>
          ...
          <div ref="payerRef" class="relative">
            ...
            >
              <template v-if="selectedPayer">
                <div
                  class="w-8 h-8 rounded-full bg-cerulean-50 shrink-0 overflow-hidden flex items-center justify-center"
                >
                  <img
                    v-if="selectedPayer.user?.avatar?.url"
                    :src="selectedPayer.user.avatar.url"
                    :alt="selectedPayer.user?.name"
                    class="w-full h-full object-cover"
                  />
                  <span v-else class="material-symbols-outlined text-brand-primary text-[16px]"
                    >person</span
                  >
                </div>
              ...
              <template v-else>
                <span
                  class="material-symbols-outlined text-cerulean-500 text-xl shrink-0"
                  >person</span
                >
            ...
            >
              <button
                v-for="m in members.filter((m) => m.status === 'active')"
                :key="m.id"
                type="button"
                @click="selectPayer(m)"
              ...
              >
                ...
                <span class="text-sm font-semibold text-cerulean-800 truncate">{{
                  m.user?.name
                }}</span>
                <span
                  v-if="payer_id === m.id"
                  class="material-symbols-outlined ml-auto text-brand-primary text-lg"
                  >check</span
                >
              </button>
            </div>
      ...
      <div class="flex flex-col gap-4">
        ...
        <!-- Total indicator (fixed / percentage) -->
        <div
          v-if="splitStrategy === 'fixed' && amount"
          class="bg-cerulean-50 rounded-2xl px-5 py-4 flex flex-col gap-2"
        >
          <div class="flex items-center justify-between">
            <span class="text-xs font-bold text-cerulean-800/60 uppercase tracking-wide"
              >Allocated</span
            >
            <span
              class="text-sm font-extrabold"
              :class="
                Math.abs(fixedRemaining) < 0.01
                  ? 'text-emerald-600'
                  : fixedRemaining < 0
                    ? 'text-red-500'
                    : 'text-cerulean-700'
          ...
          <p
            v-if="Math.abs(fixedRemaining) >= 0.01"
            class="text-xs font-semibold"
            :class="fixedRemaining > 0 ? 'text-cerulean-600' : 'text-red-500'"
          >
            {{
              fixedRemaining > 0
                ? `$${fixedRemaining.toFixed(2)} remaining`
                : `Exceeded by $${Math.abs(fixedRemaining).toFixed(2)}`
            }}
          </p>
        </div>

        <div
          v-if="splitStrategy === 'percentage'"
          class="bg-cerulean-50 rounded-2xl px-5 py-4 flex flex-col gap-2"
        >
          <div class="flex items-center justify-between">
            <span class="text-xs font-bold text-cerulean-800/60 uppercase tracking-wide"
            ...
            <span
              class="text-sm font-extrabold"
              :class="
                Math.abs(percentageRemaining) < 0.1
                  ? 'text-emerald-600'
          ...
          <p
            v-if="Math.abs(percentageRemaining) >= 0.1"
            class="text-xs font-semibold"
            :class="percentageRemaining > 0 ? 'text-cerulean-600' : 'text-red-500'"
          >
            {{
              percentageRemaining > 0
                ? `${percentageRemaining.toFixed(1)}% remaining`
                : `Exceeded by ${Math.abs(percentageRemaining).toFixed(1)}%`
            }}
          </p>
        </div>
        ...
        <div class="flex flex-col gap-2">
          <div
            ...
            :key="p.membership_id"
            class="rounded-2xl px-4 py-3 flex items-center gap-3 transition-colors"
          ...
          >
            ...
            <button
              ...
              :class="
                p.checked
                  ...
                  : 'bg-cerulean-100 text-transparent hover:bg-cerulean-200'
              "
            >
            ...
            <!-- Avatar -->
            <div
              class="w-9 h-9 rounded-full bg-cerulean-50 shrink-0 overflow-hidden flex items-center justify-center"
            >
              <img
                v-if="p.user?.avatar?.url"
                :src="p.user.avatar.url"
                :alt="p.user?.name"
                class="w-full h-full object-cover"
              />
              <span v-else class="material-symbols-outlined text-brand-primary text-[18px]"
                >person</span
              >
            </div>
            ...
            <span
              ...
              :class="p.checked ? 'text-brand-text' : 'text-brand-disabled'"
            >
              {{ p.user?.name }}
            </span>

            <!-- Split value — depends on strategy -->
            <div class="ml-auto shrink-0 text-right">
              <!-- Equal: readonly badge -->
              <span
              ...
              <div v-else-if="splitStrategy === 'fixed'" class="flex items-center gap-1">
                <span class="text-sm font-bold text-cerulean-500">$</span>
                <input
              ...
              <div
                v-else-if="splitStrategy === 'percentage'"
                class="flex items-center gap-2 sm:gap-3 w-28 sm:w-48"
              >
                <input
                ...
                />
                <span
                  class="text-sm font-extrabold tabular-nums w-12 text-right"
                  :class="p.checked ? 'text-cerulean-700' : 'text-brand-disabled'"
                >
        ...
        <!-- Split validation hint -->
        <p
          v-if="!splitValid && selectedCount > 0 && amount"
          class="text-xs font-semibold text-amber-600 pl-1"
        >
          <span class="material-symbols-outlined text-[14px] align-middle">info</span>
          {{
            splitStrategy === 'fixed'
              ? 'Allocate the full amount across participants'
      ...
      <div class="flex gap-3">
        ...
        >
          <span v-if="isSubmitting" class="material-symbols-outlined text-xl animate-spin"
...
.percentage-slider:disabled::-webkit-slider-thumb {
  background: #97bfce;
}

.percentage-slider:disabled::-moz-range-thumb {
  background: #97bfce;
}
</style>